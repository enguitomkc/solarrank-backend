...
export class UserService {
  static async createUser(name: string, email: string, password: string): Promise<UserPublic> {
  ...
  static async getUserById(id: string): Promise<UserPublic | null> {
    ...
    try {
      const result = await client.query(
        'SELECT id, name, email, profile_image, role, total_energy, created_at FROM users WHERE id = $1',
        [id]
      );
  ...
  static async getUserByEmail(email: string): Promise<UserPublic | null> {
    ...
    try {
      const result = await client.query(
        'SELECT id, name, email, profile_image, role, total_energy, created_at FROM users WHERE email = $1',
        [email]
      );
